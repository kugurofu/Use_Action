...
class GPSYamlNode(Node):
    def __init__(self):
        super().__init__('gps_yaml_node')
    ...
    def save_to_yaml(self, waypoints):
        ...
        with open("waypoints.yaml", "w") as file:
            yaml.dump(yaml_data, file, default_flow_style=False, allow_unicode=True)
        self.get_logger().info("waypoints.yaml に保存しました。")
